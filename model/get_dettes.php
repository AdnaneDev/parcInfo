...
if ($result->rowCount() > 0) { // Utilisez rowCount() au lieu de num_rows
    while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
        ...
        $fournisseurDette = array(
            "fournisseur" => $row["nom_fournisseur"],
            "dette" => floatval($row["dette"]) // Assurez-vous que la dette est un nombre
        );

        // Ajoutez ce tableau au tableau principal
        $fournisseursDettes[] = $fournisseurDette;
    }
}
...
$jsonData = json_encode($fournisseursDettes);
